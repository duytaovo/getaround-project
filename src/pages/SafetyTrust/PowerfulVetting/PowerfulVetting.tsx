      ...
      <div className='w-full flex-col items-start  flex relative '>
        <div className='flex-row w-full max-w-[1080px] mx-auto flex' style={{ gridColumnGap: '26px' }}>
          ...
          <div className='items-start flex w-full flex-col relative '>
            <CarOutlined className='text-mainColor font-normal text-[32px] mb-6' />
            <h2 className='pb-0 text-mainColor text-left uppercase m-[0_0_24px] text-[30px] font-bold leading-[32px]'>
              Powerful vetting & fraud detection system
            </h2>
            <ul>
              {Data.map((item, index) => (
                <li className='text-white mb-3'>
                  <span className='text-mainColor'> {Data?.[index]?.titleSpan} </span>
                  {Data?.[index]?.titleH2}
                </li>
              ))}
            </ul>
