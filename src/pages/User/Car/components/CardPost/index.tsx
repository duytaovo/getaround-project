...
const CardPost = (props: any) => {
  ...
  const handleOnClick = () => {
    if (props.isDemandPost) {
      ...
      //   case 'update':
      //     setComponent(<PostPartnerForm idDemand={props.idDemand} isUpdate={true} setOpen={setOpen} />)
      //     setTitle('Chỉnh sửa thông tin đợt thực tập')
      //     break
      //   case 'Đóng việc':
      //     setComponent(
      //       <Confirmation setOpen={setOpen} text='Bạn có chắc muốn đóng việc?' nameBtnYes='Đóng việc' nameBtnNo='Hủy' />
      //     )
      //     setTitle('Đóng việc')
      //     break
      //   default:
      ...
      //     break
      //   case 'close':
      //     setTitle(props.isDisabled ? 'Chỉnh sửa thông tin đăng tuyển' : 'Đóng việc')
      //     setComponent(
      //       props.isDisabled ? (
      //         <PostJobForm
      //           formStatus={'repost'}
  ...
  return (
    <div className='card-post__container'>
      ...
      <p className='card-post__time'>
        <b>Thời gian tuyển dụng:</b> {moment(props.timeStart).format('DD/MM/YYYY')} -{' '}
      ...
      <Modal modalTitle={title} name='list-candidate' open={open} setOpen={setOpen} iconClose={true}>
        {component}
      </Modal> */}
    </div>
  )
