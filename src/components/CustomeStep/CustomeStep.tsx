...
const CustomeStep: FC<Props> = ({ items, classname, icons, textClassName, titleClassName }) => {
  ...
  const itemsForSteps = items.map((item, index) => ({
    icon: icons[index],
    title: <Text id={item.title} tag='h2' content={data[item.title]} className={titleClassName} />,
    subTitle: (
      <Text id={item.subTitle} tag='p' content={data[item.subTitle]} className={textClassName} />
